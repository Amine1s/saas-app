...
const JWT_SECRET =
  process.env.JWT_SECRET || "saas_inventory_jwt_secret_2026_x89q3m4";
...
function verifyPassword(
  ...
  if (
    storedPasswordHashOrPlain.startsWith("$2a$") ||
    storedPasswordHashOrPlain.startsWith("$2b$") ||
    storedPasswordHashOrPlain.startsWith("$2y$")
  ) {
    try {
...
const requireAuth = (
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    res
      ...
      .json({
        success: false,
        message: "غير مصرح: يلزم تسجيل الدخول للمتابعة.",
      });
    return;
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "رمز الدخول (Token) غير صالح أو منتهي الصلاحية.",
      });
  }
};
  ...
  requireAuth(req, res, () => {
    ...
    if (!user || user.role !== "admin") {
      res
        ...
        .json({
          success: false,
          message: "صلاحية غير كافية: هذا الإجراء مقتصر على مدير النظام فقط.",
        });
      return;
    }
    next();
...
let warehouses: Warehouse[] = [
  {
    ...
    capacity: 5000,
    description: "مستودع السلع الأساسية والمكيفة",
  },
  {
    id: "WH-02",
    ...
    capacity: 3000,
    description: "مستودع المنتجات المستوردة والأحذية",
  },
];
...
let suppliers: Supplier[] = [
  {
    id: "SUP-01",
    name: "شركة المنسوجات الموحدة",
    company: "المصنع السعودي للملابس",
    phone: "0501112222",
    email: "info@unifiedtextiles.com",
  },
  {
    id: "SUP-02",
    name: "مؤسسة خطوات التقنية",
    company: "شركة الاستيراد العالمية",
    phone: "0503334444",
    email: "sales@techsteps.sa",
  },
];
...
let categories: Category[] = [
  {
    id: "CAT-01",
    name: "رجالي",
    description: "ملابس وأحذية وإكسسوارات رجالية تمتاز بالجودة العالية",
  },
  {
    id: "CAT-02",
    name: "نسائي",
    description: "ملابس فخمة وفساتين سهرة وتصاميم عصرية للمناسبات",
  },
  {
    id: "CAT-03",
    name: "أحذية",
    description: "أحذية رياضية ورسمية مريحة ومقاومة للماء",
  },
];
...
let stockMovements: StockMovement[] = [
  {
    ...
    type: "in",
    productId: "1",
    productName: "قميص بولو Ralph Lauren",
    quantity: 45,
    warehouseId: "WH-01",
    warehouseName: "المستودع الرئيسي - الرياض",
    notes: "توريد دفعة إنتاج جديدة من المصنع",
    timestamp: "2026-06-10 09:00",
    recordedBy: "خالد أحمد (مدير المستودع)",
  },
  {
    id: "MOV-02",
    type: "in",
    productId: "2",
    productName: "بنطال جينز Levi's 511",
    quantity: 15,
    warehouseId: "WH-01",
    warehouseName: "المستودع الرئيسي - الرياض",
    notes: "توريد ملابس من المورد الشريك",
  ...
  },
  {
    ...
    warehouseId: "WH-01",
    warehouseName: "المستودع الرئيسي - الرياض",
    notes: "تلف شحنة أو تحويل فرعي",
    timestamp: "2026-06-11 11:30",
    recordedBy: "خالد أحمد (مدير المستودع)",
  },
];
...
let products: Product[] = [
  {
    id: "1",
    name: "قميص بولو Ralph Lauren",
    sku: "PROD-001",
    price: 150.0,
    quantity: 45,
    description: "رجالي | مقاس XL | اللون أزرق",
    category: "رجالي",
    warehouseId: "WH-01",
    supplierId: "SUP-01",
  },
  {
    id: "2",
    name: "بنطال جينز Levi's 511",
    sku: "PROD-002",
    price: 220.0,
    quantity: 4,
    description: "رجالي | مقاس 32 | اللون أسود",
    category: "رجالي",
    warehouseId: "WH-01",
    supplierId: "SUP-01",
  },
  {
    id: "3",
    name: "فستان شانيل Chanel حرير",
    sku: "PROD-003",
    price: 450.0,
    quantity: 18,
    description: "نسائي | مقاس M | اللون أحمر",
    category: "نسائي",
    warehouseId: "WH-02",
    supplierId: "SUP-01",
  },
  {
    id: "4",
    name: "حذاء نايكي Nike Air Max",
    sku: "PROD-004",
    price: 300.0,
    quantity: 7,
    description: "أحذية | مقاس 42 | اللون أبيض",
    category: "أحذية",
    warehouseId: "WH-02",
    supplierId: "SUP-02",
  },
...
let activities: StoreActivity[] = [
  {
    ...
    type: "add_invoice",
    message:
      "إصدار فاتورة مبيعات رقم INV-2026-02 للمشترية سارة عبد الرحمن بقيمة 517.50 ر.س شاملة الضريبة.",
    timestamp: "قبل ساعة",
    meta: "INV-2026-02",
  },
  {
    ...
    type: "add_product",
    message:
      'إضافة صنف منتج جديد "حذاء نايكي Nike Air Max" بالرمز SKU الحالي PROD-004 ومخزون 7 قطع.',
  ...
  },
  {
    id: "3",
    type: "stock_update",
    message: 'تحديث مخزون "بنطال جينز Levi\'s 511" يدوياً إلى 4 قطع.',
  ...
  },
  {
    ...
    type: "add_invoice",
    message:
      "إصدار فاتورة مبيعات رقم INV-2026-01 للعميل أحمد مصطفى بمبلغ 425.50 ر.س.",
    timestamp: "قبل يومين",
    meta: "INV-2026-01",
  },
  {
    id: "5",
    type: "system",
    message: "تم إقران مستودع المتاجر السحابية وإدارة الأرصدة والفوترة بنجاح.",
    timestamp: "منذ أسبوع",
    meta: "SYSTEM-START",
  },
];
...
function updateChartStatistics(
  ...
  if (dayIndex !== -1) {
    if (isRefund) {
      weeklyChartPoints[dayIndex].sales = Math.max(
        0,
        weeklyChartPoints[dayIndex].sales - amount,
      );
      weeklyChartPoints[dayIndex].invoices = Math.max(
        0,
  ...
  const monthIndex = monthlyChartPoints.findIndex(
    (p) => p.label === thisMonthLabel,
  );
  if (monthIndex !== -1) {
    if (isRefund) {
      monthlyChartPoints[monthIndex].sales = Math.max(
        0,
        monthlyChartPoints[monthIndex].sales - amount,
      );
      monthlyChartPoints[monthIndex].invoices = Math.max(
        0,
...
async function syncFromFirestore() {
  ...
  if (!db) {
    console.log(
      "[Database] Running with In-Memory store (Firestore credentials not set).",
    );
    return;
  }
  try {
    ...
    console.log(
      `[Firestore] Successfully connected and synced: ${products.length} products, ${invoices.length} invoices, ${users.length} users.`,
    );
  } catch (err) {
    console.warn(
      "[Firestore] Sync warning, proceeding with in-memory state:",
      err,
    );
  }
}
...
async function startServer() {
  ...
  app.post("/api/products", (req, res) => {
    ...
    if (
      products.some((p) => p.sku.toUpperCase() === sku.trim().toUpperCase())
    ) {
      res.status(409).json({
        success: false,
  ...
  app.put("/api/products/:id", (req, res) => {
    ...
    if (
      products.some((p) => p.id !== id && p.sku.toUpperCase() === trimmedSku)
    ) {
      res.status(409).json({
        success: false,
    ...
    const updatedProduct: Product = {
      ...
      quantity: newQty,
      description:
        description !== undefined ? description.trim() : oldProduct.description,
  ...
  app.delete("/api/products/:id", (req, res) => {
    ...
    if (index === -1) {
      res
        .status(404)
        .json({ success: false, message: "المنتج المطلوب حذفه غير موجود." });
      return;
    }
    ...
    res.json({
      success: true,
      message: `تم حذف المنتج "${deleted.name}" بنجاح.`,
    });
  });
  ...
  app.post("/api/invoices", (req, res) => {
    ...
    if (newInvoice.status !== "refunded") {
      updateChartStatistics(
        Number((newInvoice.totalAmount * 1.15).toFixed(2)),
        1,
        false,
      );
    }
  ...
  app.patch("/api/invoices/:id/status", (req, res) => {
    ...
    if (previousInvoice.status !== "refunded" && status === "refunded") {
      updateChartStatistics(
        Number((previousInvoice.totalAmount * 1.15).toFixed(2)),
        1,
        true,
      );
    }
    ...
    invoices[invoiceIndex] = {
      ...
      amountPaid:
        amountPaid !== undefined
          ? Number(amountPaid)
          : invoices[invoiceIndex].amountPaid,
    };

    setFirestoreDoc(
      "invoices",
      invoices[invoiceIndex].id,
      invoices[invoiceIndex],
    );
  ...
  app.delete("/api/activities/:id", (req, res) => {
    ...
    if (index === -1) {
      res
        .status(404)
        .json({ success: false, message: "سجل العملية غير موجود." });
      return;
    }
  ...
  app.delete("/api/activities", async (req, res) => {
    ...
    res.json({
      success: true,
      message: "تم مسح كامل سجل العمليات والرقابة الفنية بنجاح.",
    });
  });
  ...
  app.delete("/api/warehouses/:id", (req, res) => {
    ...
    if (index === -1) {
      res
        .status(404)
        .json({ success: false, message: "المستودع المطلوب حذفه غير موجود." });
      return;
    }
    ...
    res.json({
      success: true,
      message: `تم حذف المستودع "${deleted.name}" بنجاح.`,
    });
  });
  ...
  app.get("/api/suppliers/:id", (req, res) => {
    ...
    if (!sup) {
      res
        .status(404)
        .json({ success: false, message: "المورد المطلوب غير موجود." });
      return;
    }
  ...
  const updateSupplierHandler = (req: any, res: any) => {
    ...
    if (supIndex === -1) {
      res
        .status(404)
        .json({ success: false, message: "المورد المطلوب تعديله غير موجود." });
      return;
    }
    ...
    const updatedSupplier: Supplier = {
      ...
      name: name.trim(),
      company:
        company !== undefined ? company.trim() : suppliers[supIndex].company,
  ...
  app.get("/api/customers/:id", (req, res) => {
    ...
    if (!cust) {
      res
        .status(404)
        .json({ success: false, message: "العميل المطلوب غير موجود." });
      return;
    }
  ...
  const updateCustomerHandler = (req: any, res: any) => {
    ...
    if (custIndex === -1) {
      res
        .status(404)
        .json({ success: false, message: "العميل المطلوب تعديله غير موجود." });
      return;
    }
  ...
  app.post("/api/categories", (req, res) => {
    const userRole =
      (req.headers["x-user-role"] as string) || req.body?.userRole;
    if (userRole === "cashier") {
      res
        .status(403)
        .json({
          success: false,
          message: "عفواً، لا يملك حساب الكاشير صلاحية إنشاء تصنيفات جديدة.",
        });
      return;
    }
  ...
  app.delete("/api/categories/:id", (req, res) => {
    ...
    if (catIndex === -1) {
      res
        .status(404)
        .json({ success: false, message: "التصنيف المطلوب حذفه غير موجود." });
      return;
    }
    ...
    res.json({
      success: true,
      message: `تم حذف التصنيف "${deleted.name}" بنجاح.`,
    });
  });
  ...
  app.post("/api/stock-movements", (req, res) => {
    const { type, productId, quantity, warehouseId, notes, recordedBy } =
      req.body;
    if (!type || !productId || quantity === undefined || !warehouseId) {
      res
        .status(400)
        .json({ success: false, message: "معطيات حركة المخزون ناقصة." });
      return;
    }
    ...
    if (type === "out" && prod.quantity < qtyNum) {
      res
        ...
        .json({
          success: false,
          message: "الكمية المطلوبة للصرف غير متوفرة بالكامل بالمخزن.",
        });
      return;
    }
  ...
  app.delete("/api/stock-movements/:id", (req, res) => {
    ...
    if (index === -1) {
      res
        .status(404)
        .json({ success: false, message: "حركة المخزون غير موجودة." });
      return;
    }
  ...
  app.delete("/api/stock-movements", async (req, res) => {
    ...
    res.json({
      success: true,
      message: "تم مسح كامل سجل حركات المخزون بنجاح.",
    });
  });
  ...
  app.post("/api/users", requireAdmin, (req, res) => {
    ...
    if (!username || !password || !name || !role) {
      res
        ...
        .json({
          success: false,
          message:
            "جميع الحقول الأساسية مطلوبة (اسم المستخدم، كلمة المرور، الاسم، والدور).",
        });
      return;
    }
    ...
    if (role === "admin") {
      res.status(403).json({
        success: false,
        message:
          "غير مسموح بإنشاء مستخدمين بصلاحية مدير نظام. تقتصر الصلاحيات المتاحة على مدير مستودع أو كاشير فقط.",
      });
      return;
    }
    ...
    if (users.some((u) => u.username.toLowerCase() === cleanUsername)) {
      res
        .status(409)
        .json({ success: false, message: "اسم المستخدم هذا مستخدم مسبقاً." });
      return;
    }
  ...
  app.put("/api/users/:id", requireAuth, (req, res) => {
    ...
    if (authUser?.role !== "admin" && authUser?.id !== id) {
      res
        ...
        .json({
          success: false,
          message: "غير مصرح لك بتعديل بيانات هذا الحساب.",
        });
      return;
    }
    ...
    if (username) {
      ...
      if (
        users.some((u) => u.id !== id && u.username.toLowerCase() === cleanU)
      ) {
        res
          .status(409)
          .json({ success: false, message: "اسم المستخدم مسجل لشخص آخر." });
        return;
      }
  ...
  app.patch("/api/users/:id/status", requireAdmin, (req, res) => {
    ...
    if (
      ...
      res
        ...
        .json({
          success: false,
          message: "لا يمكن حظر حساب مدير النظام الوحيد الفعّال.",
        });
      return;
    }
  ...
  app.delete("/api/users/:id", requireAdmin, (req, res) => {
    ...
    if (
      ...
      res
        ...
        .json({
          success: false,
          message: "لا يمكن حذف حساب مدير النظام الرئيسي.",
        });
      return;
    }
    ...
    res.json({
      success: true,
      message: `تم حذف الحساب "${deleted.name}" بنجاح.`,
    });
  });
  ...
  app.post("/api/auth/login", loginRateLimiter, (req, res) => {
    ...
    if (!matchedUser.isActive) {
      res.status(403).json({
        success: false,
        message:
          "تم حظر هذا الحساب من قبل مدير النظام. يرجى التواصل مع الإدارة لتفعيله.",
      });
      return;
    }
    ...
    if (
      ...
      !matchedUser.password.startsWith("$2a$") &&
      !matchedUser.password.startsWith("$2b$")
    ) {
  ...
  app.get("/", (req, res) => {
    res.json({
      status: "online",
      message: "SaaS Inventory ERP API Server is running successfully.",
      timestamp: new Date().toISOString(),
    });
  });
  ...
  if (process.env.NODE_ENV !== "production" || !process.env.VERCEL) {
    ...
    app.listen(PORT, "0.0.0.0", () => {
      console.log(
        `[Backend Server] Server running successfully on port ${PORT}`,
      );
    });
  }
}
