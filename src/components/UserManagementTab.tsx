...
export const UserManagementTab: React.FC<UserManagementTabProps> = ({
  ...
  const handleToggleStatus = async (user: AppUser) => {
    ...
    try {
      const res = await authFetch(`${API_BASE}/api/users/${user.id}/status`, {
        method: 'PATCH'
      });
      const data = await res.json();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className={`w-full max-w-lg rounded-2xl border p-4 sm:p-6 shadow-2xl transition-all my-auto max-h-[92vh] overflow-y-auto custom-scrollbar ${
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              {/* حالة الحساب وتفعيل الحساب فوراً */}
              <div className={`p-3.5 rounded-2xl border transition-all ${
                formData.isActive 
                  ? (darkMode ? 'bg-emerald-500/10 border-emerald-500/40' : 'bg-emerald-50 border-emerald-300')
                  : (darkMode ? 'bg-rose-500/10 border-rose-500/40' : 'bg-rose-50 border-rose-300')
              }`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ${
                      ...
                      {formData.isActive ? <Unlock className="h-5 w-5" /> : <Lock className="h-5 w-5" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-black ${
                          ...
                          {formData.isActive ? 'تفعيل الحساب فوراً (نشط)' : 'حظر وتعطيل الحساب (موقوف)'}
                        </span>
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        {formData.isActive 
                          ? 'الحساب نشط ويستطيع المشغل تسجيل الدخول فوراً باستخدام اسمه وكلمة مروره.'
                          : 'الحساب معطّل ومحظور مؤقتاً، ولن يُسمح له بتسجيل الدخول للنظام.'}
                      </p>
                    </div>
                  </div>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer select-none">
                    <input 
                      ...
                      disabled={isEditing && users.find(u => u.id === editingUserId)?.role === 'admin'}
