...
import { Edit3, X, Save, AlertCircle, Users, Phone, Mail, FileText } from 'lucide-react';
...
interface EditCustomerModalProps {
  ...
  onSave: (id: string, name: string, phone: string, email: string, taxNumber: string) => Promise<void>;
}
...
export default function EditCustomerModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError('يرجى إدخال اسم العميل أو المؤسسة التجارية.');
      return;
    }
    if (!phone.trim()) {
    ...
    try {
      setIsSaving(true);
      await onSave(customer.id, name.trim(), phone.trim(), email.trim(), taxNumber.trim());
      onClose();
    } catch (err: any) {
      setError(err?.message || 'فشل حفظ التعديلات على بيانات العميل.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <AnimatePresence>
      <div 
        id="edit-customer-modal-container"
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className={`text-base font-bold ${darkMode ? 'text-emerald-300' : 'text-[#064E3B]'}`}>
                  تعديل بيانات العميل التجاري (مدير النظام)
                </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <div>
              <label className={`block text-xs font-semibold mb-1 ${darkMode ? 'text-emerald-300' : 'text-slate-700'}`}>
                اسم العميل / المؤسسة <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="مثال: الأستاذ هاني منصور / شركة الأفق"
              />
            </div>
            ...
            <div>
              <label className={`block text-xs font-semibold mb-1 flex items-center gap-1 ${darkMode ? 'text-emerald-300' : 'text-slate-700'}`}>
                ...
                <span>الرقم الضريبي (اختياري للفوترة)</span>
              </label>
              <input
                type="text"
                value={taxNumber}
